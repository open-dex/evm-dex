...
let erc777 = buildContract('../ERC777.json');
...
async function run() {
    ...
    if (updateBoomflow || updateCRCL) {
        console.log(`\nAdd Boomflow to CRCL Whitelisted`)
        await Promise.all(
            crcl_assets.map(async (token, index) => {
                if (token.crclAddress === undefined) {
                    // noinspection SqlResolve
                    const result = await executeSql(`select contract_address from t_currency where id = '${token.id}'`);
                    if (result.length === 0) throw new Error(`crcl address not found for ${token.name}, consider deploy crcl first.`)
                    token.crclAddress = result[0]['contract_address'];
                    log(`load crcl address of ${token.name} from database, got ${token.crclAddress}`);
                }
                return addWhitelisted(token, boomflowAddr, 'addWhitelisted', nonce++);
            })
        )
        log();
    }
    if (updateCRCL) {
        // add dex admin to WhitelistAdmin and Whitelisted of CRCL contract
        console.log(`\nAdd DEX Admin to CRCL WhitelistAdmin and Whitelisted, current nonce ${nonce}`)
        let wait_list = [];
        for (let i = 0; i < crcl_assets.length; ++i) {
            wait_list.push(addWhitelisted(crcl_assets[i], dexAdmin, 'addWhitelisted', nonce++));
            wait_list.push(addWhitelisted(crcl_assets[i], dexAdmin, 'addWhitelistAdmin', nonce++));
        }
...
async function deployCRCL(token, useNonce) {
    ...
    const contract = await crcl.deploy(`CRCL ${name}`/*name*/, `${name}`/*symbol*/,
        18, erc777_addr, 259200, token.isCFX, {nonce: useNonce})
    return contract.deployTransaction.wait()
        .then((receipt) => {
            const retAddr = receipt.contractAddress;
