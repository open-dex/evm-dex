...
public class EvmAddress extends Address {
    private String address;  // base32Check address
    private String hexAddress;
    private int netId;
    private AddressType addressType;


    public EvmAddress(String str) {
        // cheat it
        super(new Address("0x1000000000000000000000000000000000000000", 1).getAddress());

        // real value
        String hexAddress = Address.decode(str);
        netId = decodeNetId(str.split(":")[0]);
        this.address = conflux.web3j.types.Address.encode(hexAddress, netId);
        this.addressType = addressType(hexAddress);
        this.hexAddress = hexAddress;
    }

    public EvmAddress(String hexAddress, int netId) throws AddressException {
        //only for grammar
        super("0x1000000000000000000000000000000000000000", netId);
        //
        this.address = conflux.web3j.types.Address.encode(hexAddress, netId);
        this.addressType = addressType(hexAddress);
        this.netId = netId;
...
//        return String.format(formatStr, this.netId, this.hexAddress, this.address);
    ...
    public static boolean isValid(String cfxAddressStr) {
        try {
            conflux.web3j.types.Address.decode(cfxAddressStr);
            return true;
        } catch(Exception e) {
            return false;
        }
